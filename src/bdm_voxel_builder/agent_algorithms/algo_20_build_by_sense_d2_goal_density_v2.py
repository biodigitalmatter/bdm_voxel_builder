...
from bdm_voxel_builder.helpers.file import get_savepath
...
class Algo20_Build_d2(AgentAlgorithm):
    ...
    def update_environment(self, state: Environment):
        ...
        # update goal_density
        emission_array = (
            goal_density.to_numpy() * follow_goal
            + built_volume.to_numpy() * follow_built
        )
    ...
    def get_move_values(self, agent: Agent, state: Environment):
        return (
            agent.calculate_move_values_random__z_based__follow_with_low_density_weight(
                state.grids["follow"], state.grids["built"]
            )
        )
