...
from bdm_voxel_builder.grid import NumpyGrid
...
def test_to_vdb_grid(random_int_array):
    grid = NumpyGrid(array=random_int_array, grid_size=random_int_array.shape)
...
def test_get_local_bbox():
    grid = NumpyGrid(grid_size=(10, 10, 10))
    assert grid.bbox.diagonal == cg.Line((0, 0, 0), (10, 10, 10))


def test_get_local_bbox_with_non_square_grid_size():
    grid = NumpyGrid(grid_size=(5, 10, 15))

    assert grid.bbox.xsize == cg.Box(xsize=5, ysize=10, zsize=15).xsize
    assert grid.bbox.ysize == cg.Box(xsize=5, ysize=10, zsize=15).ysize
    assert grid.bbox.zsize == cg.Box(xsize=5, ysize=10, zsize=15).zsize
...
def test_get_world_bbox_with_non_square_grid_size():
    ...
    grid = NumpyGrid(grid_size=(5, 5, 5), xform=T)
    bbox = grid.get_world_bbox()
    assert bbox.diagonal == cg.Line((15, 10, 5), (20, 15, 10))


def test_from_vdb_w_file():
    grid = NumpyGrid.from_vdb(get("sphere.vdb"))
...
def test_get_world_pts(activate_random_indices, random_generator):
    grid = NumpyGrid(grid_size=(10, 10, 10), xform=cg.Scale.from_factors([2, 2, 2]))
    ...
    assert grid.bbox.contains_points(world_pts)


def test_get_world_pointcloud(activate_random_indices, random_generator):
    grid = NumpyGrid(
        grid_size=(10, 10, 10), xform=cg.Translation.from_vector([2, 2, 4])
    )
...
def test_get_merged_array_with():
    grid1 = NumpyGrid(grid_size=(10, 10, 10))
    grid2 = NumpyGrid(grid_size=(10, 10, 10))
    merged_array = grid1.merged_with(grid2)
...
def test_from_pointcloud():
    ...
    grid = NumpyGrid.from_pointcloud(pointcloud, grid_size=3)
...
def test_from_pointcloud_large(random_pts, random_generator):
    ...
    grid = NumpyGrid.from_pointcloud(pointcloud, grid_size=25)
...
class TestFromPcd:
    ...
    def test_from_pcd_ascii(self, ascii_pcd, parsed_ascii_pcd):
        grid = NumpyGrid.from_pcd(ascii_pcd, grid_size=100)
    ...
    def test_from_pcd_binary(self, binary_pcd, parsed_ascii_pcd):
        grid = NumpyGrid.from_pcd(binary_pcd, grid_size=100)

        assert grid.array.shape == (100, 100, 100)
        assert_allclose(grid.get_world_pts(), parsed_ascii_pcd)

    def test_from_pcd_binary_compressed(self, binary_compressed_pcd, parsed_ascii_pcd):
        grid = NumpyGrid.from_pcd(binary_compressed_pcd, grid_size=100)
